...
static void rotation_log(const char *fmt, ...);
...
static int register_dnssd() {
    ...
    if (getenv("UXPLAY_HIDDEN")) {
        LOGI("UXPLAY_HIDDEN is set: running in hidden-backend mode, no mDNS advertisement");
        return 0;
    }
...
extern "C" void video_reset(void *cls, reset_type_t type) {
    rotation_log("video_reset called, type=%d", (int)type);
    switch (type) {
...
/* device name as reported by the AirPlay (RTSP) layer — the real iPhone
   name, independent of whatever TrollVNC's desktop-name setting says */
static char g_ap_name[128] = {0};
static void backend_notify(const char *msg);
extern char vncm_ip[64];
...
extern "C" void report_client_request(void *cls, char *deviceid, char * model, char *name, bool * admit) {
    ...
    if (name && *name) {
        strncpy(g_ap_name, name, sizeof(g_ap_name) - 1);
        g_ap_name[sizeof(g_ap_name) - 1] = 0;
        if (vncm_ip[0]) {
            char m[300];
            _snprintf(m, sizeof(m), "aname %s %s", vncm_ip, g_ap_name);
            backend_notify(m);
        }
    }
    if (restrict_clients) {
...
extern "C" void video_pause (void *cls) {
    rotation_log("video_pause called");
    if (use_video) {
...
extern "C" void video_resume (void *cls) {
    rotation_log("video_resume called");
    if (use_video) {
...
static DWORD WINAPI vncm_probe_once(LPVOID p);
static void vncm_mark_lost(const char *why);
static void backend_notify(const char *msg);
extern CRITICAL_SECTION vncm_cs;
extern volatile LONG vncm_tried;
extern char vncm_ip[64];
extern volatile LONG vncm_connected;
extern SOCKET vncm_sock;

/* last mirror-stream dimensions reported by the device (rotation aware) */
static volatile LONG g_stream_w = 0, g_stream_h = 0;

static void rotation_log(const char *fmt, ...) {
    char path[MAX_PATH];
    /* write to %LOCALAPPDATA%\rotation.log — always writable */
    if (GetEnvironmentVariableA("LOCALAPPDATA", path, sizeof(path) - 20)) {
        strcat(path, "\\rotation.log");
    } else if (GetEnvironmentVariableA("TEMP", path, sizeof(path) - 20)) {
        strcat(path, "\\rotation.log");
    } else {
        strcpy(path, "rotation.log");
    }
    FILE *f = fopen(path, "a");
    if (!f) return;
    SYSTEMTIME st;
...
extern "C" void video_report_size(void *cls, float *width_source, float *height_source, float *width, float *height) {
    ...
    if (sw > 0 && sh > 0) {
        int oldw = InterlockedExchange(&g_stream_w, sw);
        int oldh = g_stream_h;
        InterlockedExchange(&g_stream_h, sh);
        if (oldw > 0 && (oldw != sw || oldh != sh)) {
            /* Device rotated: tell RAOP layer to suppress pipeline restart
               on the upcoming TEARDOWN. The flag is cleared by re-SETUP. */
            raop_set_rotation_pending(raop, true);
            rotation_log("ROTATION DETECTED: stream %dx%d -> %dx%d, rotation_pending=true", oldw, oldh, sw, sh);
        }
        if (!vncm_ip[0]) return;
        char msg[128];
        _snprintf(msg, sizeof(msg), "dim %s %d %d", vncm_ip, sw, sh);
        backend_notify(msg);
        if (oldw > 0 && (oldw != sw || oldh != sh)) {
            /* orientation changed: TrollVNC framebuffer resized too —
               drop the stale session; next use reconnects with fresh dims.
               Do NOT touch the GStreamer pipeline — flushing or restarting
               it from a non-main thread causes freezes and data loss. */
            fprintf(stderr, "vnc: stream %dx%d -> %dx%d, recycling vnc\n", oldw, oldh, sw, sh);
            EnterCriticalSection(&vncm_cs);
            if (vncm_connected && vncm_sock != INVALID_SOCKET) vncm_mark_lost("rotate");
            vncm_tried = 0;
            LeaveCriticalSection(&vncm_cs);
            CreateThread(NULL, 0, vncm_probe_once, NULL, 0, NULL);
        }
    }
}
...
#ifdef _WIN32
/* ================= VNC reverse-control module (TrollVNC) ================= */
static int vncm_port = 5901;
static int vncm_rot_ccw = 0;
char vncm_ip[64] = {0};
SOCKET vncm_sock = INVALID_SOCKET;
static int vncm_fw = 0, vncm_fh = 0;
static char vncm_devname[128] = {0};
static int vncm_last_fx = 0, vncm_last_fy = 0;
volatile LONG vncm_connected = 0;
volatile LONG vncm_tried = 0;
CRITICAL_SECTION vncm_cs;
static HANDLE vncm_wake = NULL;
static WNDPROC vncm_orig_proc = NULL;
static HWND vncm_hwnd = NULL;
static int vncm_map_cw = 1, vncm_map_ch = 1;
static DWORD vncm_last_send = 0;

typedef struct { int kind; int mask; int x, y, dir; } vnc_ev_t;
#define VNC_QN 32
static vnc_ev_t vnc_q[VNC_QN];
static volatile LONG vnc_qh = 0, vnc_qt = 0;

static int vncm_send_all(SOCKET s, const char *b, int n);

static int vncm_rx(SOCKET s, char *b, int n) {
    int got = 0;
    while (got < n) {
...
static SOCKET vncm_connect(void) {
    SOCKET s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    SOCKADDR_IN a; memset(&a, 0, sizeof(a));
    a.sin_family = AF_INET;
    a.sin_addr.s_addr = inet_addr(vncm_ip);
    a.sin_port = htons((unsigned short) vncm_port);
    if (s == INVALID_SOCKET) return INVALID_SOCKET;
    if (connect(s, (SOCKADDR *) &a, sizeof(a)) != 0) {
        fprintf(stderr, "vnc: %s:%d connect failed (%d)\n", vncm_ip, vncm_port, WSAGetLastError());
        closesocket(s);
        return INVALID_SOCKET;
    }
    int one = 1;
    setsockopt(s, IPPROTO_TCP, TCP_NODELAY, (char *) &one, sizeof(one));
    /* keepalive: detect dead peers (phone wifi drop / app killed) in ~10s */
    {
        DWORD ka[3] = {3000, 3000, 3000};
        DWORD retb = 0;
        setsockopt(s, SOL_SOCKET, SO_KEEPALIVE, (char *) &one, sizeof(one));
#ifndef SIO_KEEPALIVE_VALS
#define SIO_KEEPALIVE_VALS _WSAIOW(IOC_VENDOR, 4)
#endif
        WSAIoctl(s, SIO_KEEPALIVE_VALS, &ka, sizeof(ka), NULL, 0, &retb, NULL, NULL);
    }
    DWORD to = 3000;
    ...
    char buf[64];
    int vmaj = 3, vmin = 8;
    if (vncm_rx(s, buf, 12) != 12) { fprintf(stderr, "vnc %s: no version\n", vncm_ip); goto fail; }
    ...
    int ntypes = 0;
    if (vmaj == 3 && vmin > 6) {
        if (vncm_rx(s, buf, 1) != 1) { fprintf(stderr, "vnc %s: no sec count\n", vncm_ip); goto fail; }
        ntypes = (unsigned char) buf[0];
        if (ntypes <= 0 || ntypes > 15) { fprintf(stderr, "vnc %s: bad sec count %d\n", vncm_ip, ntypes); goto fail; }
        if (vncm_rx(s, (char *) types, ntypes) != ntypes) goto fail;
    } else {
        unsigned int st;
        if (vncm_rx(s, (char *) &st, 4) != 4) { fprintf(stderr, "vnc %s: no sectype\n", vncm_ip); goto fail; }
        st = ((st & 0xff) << 24) | ((st & 0xff00) << 8) | ((st >> 8) & 0xff00) | (st >> 24);
        if (st == 0 || st > 2) { fprintf(stderr, "vnc %s: refused (%u)\n", vncm_ip, st); goto fail; }
    ...
    }
    {
        ...
        for (int k = 0; k < ntypes; k++)
            if (types[k] == 1) sel = 1;
        if (sel < 0) {
            fprintf(stderr, "vnc %s: None auth not offered (%d types)\n", vncm_ip, ntypes);
            goto fail;
        }
    ...
    if (vmaj == 3 && vmin > 7) {
        unsigned int res;
        if (vncm_rx(s, (char *) &res, 4) != 4) { fprintf(stderr, "vnc %s: no sec result\n", vncm_ip); goto fail; }
        if (res != 0) { fprintf(stderr, "vnc %s: sec failed\n", vncm_ip); goto fail; }
    }
    fprintf(stderr, "vnc %s: handshake ok (rfb %d.%d)\n", vncm_ip, vmaj, vmin);
    unsigned char ci = 1;
    if (send(s, (char *) &ci, 1, 0) != 1) goto fail;
    unsigned char si[24];
    if (vncm_rx(s, (char *) si, 24) != 24) { fprintf(stderr, "vnc %s: no serverinit\n", vncm_ip); goto fail; }
    vncm_fw = (si[0] << 8) | si[1];
    vncm_fh = (si[2] << 8) | si[3];
    unsigned int nl = ((unsigned) si[20] << 24) | ((unsigned) si[21] << 16) | ((unsigned) si[22] << 8) | si[23];
    vncm_devname[0] = 0;
    if (nl > 0 && nl < sizeof(vncm_devname)) {
        char nbuf[128];
        if (vncm_rx(s, nbuf, (int) nl) != (int) nl) goto fail;
        memcpy(vncm_devname, nbuf, nl);
        vncm_devname[nl] = 0;
    } else if (nl > 0 && nl < 256 && vncm_rx(s, buf, (int) nl) != (int) nl) goto fail;
    /* SetEncodings: hextile preferred */
    {
        unsigned char se[8] = {2, 0, 0, 1, 0xFF, 0xFF, 0xFF, 0xFB};
        if (vncm_send_all(s, (char *) se, 8) != 8) goto fail;
    }
    fprintf(stderr, "vnc connected %s framebuffer %dx%d\n", vncm_ip, vncm_fw, vncm_fh);
    return s;
    }
fail:
    closesocket(s);
...
static void backend_notify(const char *msg) {
    ...
    a.sin_port = htons((unsigned short)(atoi(cp) + 64));
    sendto(s, msg, (int) strlen(msg), 0, (SOCKADDR *) &a, sizeof(a));
    closesocket(s);
}

static void vncm_report_name(void) {
    /* tell router which device name this VNC session belongs to */
    if (!vncm_ip[0]) return;
    if (vncm_devname[0]) {
        char msg[300];
        _snprintf(msg, sizeof(msg), "name %s %s", vncm_ip, vncm_devname);
        backend_notify(msg);
    }
    /* the AirPlay-layer name is authoritative — repeat it on every VNC
       (re)connect so the router has it even if the RTSP notice raced */
    if (g_ap_name[0]) {
        char msg[300];
        _snprintf(msg, sizeof(msg), "aname %s %s", vncm_ip, g_ap_name);
        backend_notify(msg);
    }
}

static void vncm_ensure(void) {
    if (vncm_connected) return;
    if (!vncm_ip[0]) return;
    DWORD now = GetTickCount();
    if (vncm_tried && now - vncm_tried < 600) return;
    vncm_tried = now;
    EnterCriticalSection(&vncm_cs);
    SOCKET s = vncm_connect();
    if (s != INVALID_SOCKET) {
        vncm_sock = s;
        InterlockedExchange(&vncm_connected, 1);
        LeaveCriticalSection(&vncm_cs);
        vncm_report_name();
        return;
    }
...
/* robust send: partial writes are NOT errors — keep pushing until done or deadline */
static int vncm_send_all(SOCKET s, const char *b, int n) {
    int off = 0;
    ...
    while (off < n) {
        ...
        if (r < 0) {
            ...
            if ((e == WSAEWOULDBLOCK || e == WSAETIMEDOUT ||
                 e == WSAENOBUFS || e == WSAEINTR) &&
                GetTickCount() - t0 < 8000)
                continue;
            return -1;
        }
        if (GetTickCount() - t0 >= 8000) return -1;
    }
    return off;
}

static void vncm_pointer(int mask, int wx, int wy) {
    vncm_ensure();
    if (!vncm_connected) return;
    float xl = (float) wx / (vncm_map_cw > 0 ? vncm_map_cw : 1);
    float yl = (float) wy / (vncm_map_ch > 0 ? vncm_map_ch : 1);
    if (xl < 0) xl = 0;
    if (xl > 1) xl = 1;
    if (yl < 0) yl = 0;
    if (yl > 1) yl = 1;
    int fx, fy;
    /* the window never rotates content — it stretches or pillarboxes the
       stream, so fractions map linearly onto the framebuffer in every
       orientation (the old portrait transpose caused swapped axes) */
    fx = (int)(xl * vncm_fw);
    fy = (int)(yl * vncm_fh);
    vncm_last_fx = fx;
    vncm_last_fy = fy;
    unsigned char msg[6] = {5, (unsigned char)(mask & 0x7f),
                            (unsigned char)(fx >> 8), (unsigned char)(fx & 0xff),
                            (unsigned char)(fy >> 8), (unsigned char)(fy & 0xff)};
    EnterCriticalSection(&vncm_cs);
    if (!vncm_connected ||
        vncm_send_all(vncm_sock, (char *) msg, 6) != 6) {
        if (vncm_connected) vncm_mark_lost("pointer-send");
    }
    LeaveCriticalSection(&vncm_cs);
}

static void vncm_key(unsigned int sym, int down) {
    vncm_ensure();
    if (!vncm_connected) return;
    unsigned char msg[8] = {4, (unsigned char)(down ? 1 : 0), 0, 0, 0,
                            (unsigned char)((sym >> 24) & 0xff),
                            (unsigned char)((sym >> 16) & 0xff),
                            (unsigned char)((sym >> 8) & 0xff)};
    msg[7] = (unsigned char)(sym & 0xff);
    EnterCriticalSection(&vncm_cs);
    if (!vncm_connected || vncm_send_all(vncm_sock, (char *) msg, 8) != 8) {
        if (vncm_connected) vncm_mark_lost("key-send");
    }
    LeaveCriticalSection(&vncm_cs);
}

static void vncm_cuttext(const char *text, int len) {
    vncm_ensure();
    if (!vncm_connected || len < 0 || len > 4096) return;
    unsigned int l = (unsigned int) len;
    unsigned char hdr[8] = {6, 0, 0, 0,
                            (unsigned char)((l >> 24) & 0xff),
                            (unsigned char)((l >> 16) & 0xff),
                            (unsigned char)((l >> 8) & 0xff),
                            (unsigned char)(l & 0xff)};
    EnterCriticalSection(&vncm_cs);
    int ok = vncm_connected && vncm_send_all(vncm_sock, (char *) hdr, 8) == 8 &&
             (len == 0 || vncm_send_all(vncm_sock, text, len) == len);
    if (!ok && vncm_connected) vncm_mark_lost("cut-send");
    LeaveCriticalSection(&vncm_cs);
}

static void vncm_mark_lost(const char *why) {
    closesocket(vncm_sock);
    vncm_sock = INVALID_SOCKET;
    InterlockedExchange(&vncm_connected, 0);
    fprintf(stderr, "vnc %s: connection lost (%s err=%d)\n", vncm_ip, why, WSAGetLastError());
}

static DWORD WINAPI vncm_reader(LPVOID p) {
    (void) p;
    char junk[16384];
    DWORD last_req = 0, last_beat = GetTickCount();
    for (;;) {
        Sleep(30);
        EnterCriticalSection(&vncm_cs);
        int conn = vncm_connected;
        SOCKET s = vncm_sock;
        int fw = vncm_fw, fh = vncm_fh;
        LeaveCriticalSection(&vncm_cs);
        if (!conn || s == INVALID_SOCKET || !fw || !fh) continue;
        DWORD now = GetTickCount();
        /* NOTE: no framebuffer-update requests — input-only client.
           Requesting updates makes this TrollVNC build flood full frames,
           stalling its reader thread and blocking our pointer events. */
        if (now - last_beat >= 5000) {
            last_beat = now;
            fprintf(stderr, "vnc %s: alive %dx%d\n", vncm_ip, vncm_fw, vncm_fh);
        }
        /* idle heartbeat: harmless finger-up pointer event forces the TCP
           link to prove itself so zombie connections surface immediately */
        if (now - last_req >= 5000) {
            last_req = now;
            unsigned char hb[6] = {5, 0,
                                   (unsigned char)(vncm_last_fx >> 8), (unsigned char)(vncm_last_fx & 0xff),
                                   (unsigned char)(vncm_last_fy >> 8), (unsigned char)(vncm_last_fy & 0xff)};
            EnterCriticalSection(&vncm_cs);
            if (!vncm_connected || vncm_send_all(vncm_sock, (char *) hb, 6) != 6) {
                if (vncm_connected) vncm_mark_lost("heartbeat");
            }
            LeaveCriticalSection(&vncm_cs);
        }
        if (!vncm_connected) continue;
        u_long avail = 0;
        ioctlsocket(s, FIONREAD, &avail);
        while (avail > 0) {
            int chunk = avail > (u_long) sizeof(junk) ? (int) sizeof(junk) : (int) avail;
            int r = recv(s, junk, chunk, 0);
            if (r <= 0) {
                EnterCriticalSection(&vncm_cs);
                if (vncm_sock == s && vncm_connected) vncm_mark_lost("drain-recv");
                LeaveCriticalSection(&vncm_cs);
                break;
            }
            avail -= (u_long) r;
...
static void vncm_post(int kind, int mask, int x, int y, int dir) {
    EnterCriticalSection(&vncm_cs);
    LONG h = vnc_qh;
    if (h - vnc_qt < VNC_QN) {
        vnc_ev_t *e = &vnc_q[h % VNC_QN];
...
static DWORD WINAPI vncm_worker(LPVOID p) {
    (void) p;
    for (;;) {
        WaitForSingleObject(vncm_wake, INFINITE);
        for (;;) {
            vnc_ev_t e;
            int have = 0;
            EnterCriticalSection(&vncm_cs);
            if (vnc_qt != vnc_qh) { e = vnc_q[vnc_qt % VNC_QN]; vnc_qt++; have = 1; }
            LeaveCriticalSection(&vncm_cs);
            if (!have) break;
            if (e.kind == 0) {
                vncm_pointer(e.mask, e.x, e.y);
            } else if (e.kind == 1) {
                for (int step = -1; step <= 3; step++) {
                    int yy = e.y + e.dir * step * 25;
                    if (yy < 0) yy = 0;
                    if (yy >= vncm_map_ch) yy = vncm_map_ch - 1;
                    vncm_pointer(step == -1 ? 0 : 1, e.x, yy);
                    Sleep(15);
                }
                vncm_pointer(0, e.x, e.y + e.dir * 75);
            }
        }
    }
    return 0;
...
static LRESULT CALLBACK vncm_mouse_proc(HWND h, UINT m, WPARAM wp, LPARAM lp) {
    RECT rc;
    GetClientRect(h, &rc);
    vncm_map_cw = rc.right ? rc.right : 1;
    vncm_map_ch = rc.bottom ? rc.bottom : 1;
    short xp = (short)((lp) & 0xffff);
    short yp = (short)(((lp) >> 16) & 0xffff);
    ...
    case WM_MBUTTONDBLCLK:
        SetCapture(h);
        if (m == WM_RBUTTONDOWN || m == WM_RBUTTONDBLCLK) btnmask |= 4;
        else btnmask |= 2;
        vncm_post(0, btnmask, xp, yp, 0);
        return 0;
    case WM_MOUSEMOVE:
    ...
    case WM_RBUTTONUP:
    case WM_MBUTTONUP:
        if (m == WM_RBUTTONUP) btnmask &= ~4;
        else btnmask &= ~2;
        if (!btnmask) ReleaseCapture();
        vncm_post(0, btnmask, xp, yp, 0);
        return 0;
    case WM_LBUTTONUP:
        if (btnmask) {
            ReleaseCapture();
            btnmask = 0;
            vncm_post(0, 0, xp, yp, 0);
        }
        return 0;
    case WM_MOUSEWHEEL: {
        DWORD now = GetTickCount();
        if (now - vncm_last_send < 150) return 0;
        vncm_last_send = now;
        short delta = (short)(wp >> 16);
        vncm_post(1, 0, xp, yp, delta > 0 ? -1 : 1);
        return 0;
    }
    }
    return CallWindowProc(vncm_orig_proc, h, m, wp, lp);
}

typedef struct { HWND hwnd; } vncfind_t;

static BOOL CALLBACK vncm_win_enum(HWND h, LPARAM lp) {
    vncfind_t *ctx = (vncfind_t *) lp;
    RECT r;
    /* matches ANY window of this process incl. embedded children */
    if (!IsWindowVisible(h)) return TRUE;
    if (!GetWindowRect(h, &r) || (r.right - r.left) < 100 || (r.bottom - r.top) < 80) return TRUE;
    ctx->hwnd = h;
    return FALSE;
}

static BOOL CALLBACK vncm_thread_enum(DWORD tid, LPARAM lp) {
    return EnumThreadWindows(tid, vncm_win_enum, lp);
}
            ...
            if (snap != INVALID_HANDLE_VALUE) {
                ...
                if (Thread32First(snap, &te)) {
                    do {
                        ...
                        if (!EnumThreadWindows(te.th32ThreadID, vncm_win_enum,
                                               (LPARAM) &ctx)) break; /* found */
                    } while (Thread32Next(snap, &te));
                }
                CloseHandle(snap);
            }
            if (ctx.hwnd && !vncm_orig_proc) {
                WNDPROC op = (WNDPROC) SetWindowLongPtrW(ctx.hwnd, GWLP_WNDPROC,
                                                         (LONG_PTR) vncm_mouse_proc);
                if (op) {
                    vncm_orig_proc = op;
                    vncm_hwnd = ctx.hwnd;
                    /* apply app icon to the video window (taskbar + titlebar) */
                    HICON big = (HICON) LoadImageW(GetModuleHandleW(NULL),
                                                   MAKEINTRESOURCEW(1), IMAGE_ICON,
                                                   GetSystemMetrics(SM_CXICON),
                                                   GetSystemMetrics(SM_CYICON),
                                                   LR_DEFAULTCOLOR);
                    HICON small = (HICON) LoadImageW(GetModuleHandleW(NULL),
                                                     MAKEINTRESOURCEW(1), IMAGE_ICON,
                                                     GetSystemMetrics(SM_CXSMICON),
                                                     GetSystemMetrics(SM_CYSMICON),
                                                     LR_DEFAULTCOLOR);
                    if (big) SendMessageW(ctx.hwnd, WM_SETICON, ICON_BIG, (LPARAM) big);
                    if (small) SendMessageW(ctx.hwnd, WM_SETICON, ICON_SMALL, (LPARAM) small);
                    fprintf(stderr, "vnc: mouse capture on video window\n");
                }
            }
        }
        Sleep(500);
...
static DWORD WINAPI vncm_ctrl(LPVOID p) {
    (void) p;
    const char *cp = getenv("UXPLAY_CTRL_PORT");
    if (!cp || !*cp) { fprintf(stderr, "vnc: no UXPLAY_CTRL_PORT env\n"); return 0; }
    SOCKET s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    SOCKADDR_IN a; memset(&a, 0, sizeof(a));
    a.sin_family = AF_INET;
    a.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
    a.sin_port = htons((unsigned short) atoi(cp));
    if (bind(s, (SOCKADDR *) &a, sizeof(a)) != 0) {
        fprintf(stderr, "vnc: ctrl bind %s failed (%d)\n", cp, WSAGetLastError());
        return 0;
    }
    fprintf(stderr, "vnc: ctrl listening on 127.0.0.1:%s\n", cp);
    char b[4096];
    for (;;) {
        ...
        if (!strncmp(b, "ip ", 3)) {
            EnterCriticalSection(&vncm_cs);
            if (strcmp(vncm_ip, b + 3) != 0) {
                strncpy(vncm_ip, b + 3, sizeof(vncm_ip) - 1);
                vncm_ip[sizeof(vncm_ip) - 1] = 0;
                if (vncm_sock != INVALID_SOCKET) closesocket(vncm_sock);
                vncm_sock = INVALID_SOCKET;
                InterlockedExchange(&vncm_connected, 0);
                vncm_tried = 0;
                fprintf(stderr, "vnc: target set to %s\n", vncm_ip);
            }
            LeaveCriticalSection(&vncm_cs);
            /* connect proactively so the device name reaches the router fast */
            CreateThread(NULL, 0, vncm_probe_once, NULL, 0, NULL);
        } else if (!strncmp(b, "drop", 4)) {
            /* rejected by router (device not in selected group) */
            EnterCriticalSection(&vncm_cs);
            if (vncm_sock != INVALID_SOCKET) closesocket(vncm_sock);
            vncm_sock = INVALID_SOCKET;
            InterlockedExchange(&vncm_connected, 0);
            ...
            vncm_tried = 0;
            LeaveCriticalSection(&vncm_cs);
            fprintf(stderr, "vnc: dropped by router\n");
        } else if (b[0] == 'P' && b[1] == ' ') {
            /* API pointer event: P <mask> <x> <y> (window px).
               Refresh client size first — no local mouse may have hovered,
               and the window resizes on rotation/pillarbox changes. */
            int mask, x, y;
            if (sscanf(b + 2, "%d %d %d", &mask, &x, &y) == 3) {
                if (vncm_hwnd && IsWindow(vncm_hwnd)) {
                    RECT rc;
                    if (GetClientRect(vncm_hwnd, &rc) && rc.right > 0 && rc.bottom > 0) {
                        vncm_map_cw = rc.right;
                        vncm_map_ch = rc.bottom;
                    }
                }
                vncm_pointer(mask, x, y);
            }
        } else if (b[0] == 'W' && b[1] == ' ') {
            /* API wheel: W <dir> <count> <x> <y> */
            int dir, cnt, x, y;
            if (sscanf(b + 2, "%d %d %d %d", &dir, &cnt, &x, &y) == 4) {
                if (cnt < 1) cnt = 1;
                if (cnt > 10) cnt = 10;
                for (int i = 0; i < cnt; i++) vncm_post(1, 0, x, y, dir);
            }
        } else if (b[0] == 'K' && b[1] == ' ') {
            /* API key: K <keysym> <down> */
            unsigned int sym; int dn;
            if (sscanf(b + 2, "%u %d", &sym, &dn) == 2)
                vncm_key(sym, dn);
        } else if (b[0] == 'X' && b[1] == ' ') {
            /* API clipboard: X <raw utf8 text to end of datagram> */
            vncm_cuttext(b + 2, n - 2);
        }
    }
    return 0;
}

static void vncm_start(void) {
    const char *vp = getenv("VNC_PORT");
    if (vp && *vp) vncm_port = atoi(vp);
    const char *vr = getenv("VNC_ROT");
    vncm_rot_ccw = (vr && !_stricmp(vr, "ccw"));
    WSADATA wsa;
    if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0)
        fprintf(stderr, "vnc: WSAStartup failed (%d)\n", WSAGetLastError());
    InitializeCriticalSection(&vncm_cs);
    vncm_wake = CreateEventW(NULL, FALSE, FALSE, NULL);
    CreateThread(NULL, 0, vncm_ctrl, NULL, 0, NULL);
    CreateThread(NULL, 0, vncm_watcher, NULL, 0, NULL);
    CreateThread(NULL, 0, vncm_worker, NULL, 0, NULL);
    CreateThread(NULL, 0, vncm_reader, NULL, 0, NULL);
}
/* =============== end VNC reverse-control module =============== */
#endif
...
int main (int argc, char *argv[]) {
...
#ifdef _WIN32
    /* unbuffered stdio: router captures our output into be*.log files and
       may kill us at any moment (reject/evict) — buffered logs would vanish.
       In debug mode we also open our own log file so capture never depends
       on handle inheritance. */
    {
    ...
    {
        const char *cp = getenv("UXPLAY_CTRL_PORT");
        fprintf(stderr, "uxplay backend starting pid=%d ctrl=%s\n",
...
#define PID_MAX 4194304 // 2^22
    ...
    reconnect:
    rotation_log("reconnect: relaunch_video=%d close_window=%d preserve=%d full_reset=%d",
                  relaunch_video, close_window, preserve_connections, full_video_reset);
    compression_type = 0;
    close_window = new_window_closing_behavior;
    main_loop();
    rotation_log("main_loop exited: relaunch_video=%d reset_loop=%d", relaunch_video, reset_loop);
    if (relaunch_video) {
        if (reset_httpd) {
