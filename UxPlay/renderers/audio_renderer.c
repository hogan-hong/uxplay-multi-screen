...
static gboolean check_plugins (void)
{
    ...
    const gchar *needed[] = { "app", "playback", "videoparsersbad",  NULL};
    const gchar *gst[] = {"plugins-base", "plugins-base", "plugins-bad", NULL};
    /* optional: only used by -avdec fallback or autovideosink default */
    const gchar *optional[] = { "libav", "autodetect", NULL};
    ...
    for (int i = 0; optional[i]; i++) {
        GstPlugin *plugin = gst_registry_find_plugin (registry, optional[i]);
        if (!plugin)
            g_print ("Note: optional gstreamer plugin '%s' not present (ignored)\n", optional[i]);
        else
            gst_object_unref (plugin);
    }
    if (ret == FALSE) {
