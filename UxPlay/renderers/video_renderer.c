    ...
    for (int i = 0; i < n_renderers; i++) {
        ...
        if (hls_video) {
            ...
            if (!rtp || jpeg_pipeline) {
                ...
                g_string_append(launch, "video/x-raw,width=4096,height=4096 ! ");
                if (jpeg_pipeline) {
...
static gboolean gstreamer_video_pipeline_bus_callback(GstBus *bus, GstMessage *message, void *loop) {
    ...
    case GST_MESSAGE_ERROR: {
        ...
        /* Write error to rotation.log for diagnosis */
        {
            FILE *lf = fopen("rotation.log", "a");
            if (lf) {
                fprintf(lf, "GStreamer ERROR from [%s]: %s\n",
                        GST_MESSAGE_SRC_NAME(message), err->message);
                if (debug) fprintf(lf, "  debug: %s\n", debug);
                fclose(lf);
            }
            ...
            if (la && *la) {
                char lpath[512];
                snprintf(lpath, sizeof(lpath), "%s\\rotation.log", la);
                lf = fopen(lpath, "a");
                if (lf) {
                    fprintf(lf, "GStreamer ERROR from [%s]: %s\n",
                            GST_MESSAGE_SRC_NAME(message), err->message);
