...
struct raop_s {
    ...
    unsigned short pin;
    bool use_pin;
    volatile bool rotation_pending;
...
void raop_set_rotation_pending(raop_t *raop, bool val) {
    if (raop) raop->rotation_pending = val;
}

bool raop_is_rotation_pending(raop_t *raop) {
    return raop ? raop->rotation_pending : false;
}
