...
static int mdns_dbg_enabled = -1;
static int mdns_dbg(void)
{
    if (mdns_dbg_enabled < 0) {
        mdns_dbg_enabled = getenv("MDNS_DEBUG") ? 1 : 0;
    }
    return mdns_dbg_enabled;
}
...
{
    const char *override_ip = getenv("MDNS_IFACE_IP");

    if (override_ip && *override_ip) {
        uint32_t addr = 0;
        if (inet_pton(AF_INET, override_ip, &addr) == 1) {
            return addr;
        }
    }
                              ...
                              const struct sockaddr_in *to)
{
    ...
    if (mdns_dbg()) {
        fprintf(stderr, "[mdns-dbg] tx %d bytes to %s:%u\n",
                (int) packet->length, inet_ntoa(addr.sin_addr),
...
static THREAD_RETVAL mdns_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (ret > 0 && fd4 != -1 && FD_ISSET(fd4, &rfds)) {
            ...
            if (received > 0) {
                if (mdns_dbg()) {
                    unsigned flags = ((unsigned) buffer[2] << 8) | buffer[3];
                    fprintf(stderr, "[mdns-dbg] rx %d bytes from %s:%u QR=%u qd=%u\n",
                            received, inet_ntoa(from.sin_addr),
                            (unsigned) ntohs(from.sin_port),
                            (flags >> 15) & 1u,
                            ((unsigned) buffer[4] << 8) | buffer[5]);
                }
...
int mdnsd_start(mdnsd_t *mdnsd)
{
    ...
    if (mdns_dbg()) {
        struct in_addr dbg_a;
        dbg_a.s_addr = mdnsd->ipv4_addr;
        fprintf(stderr, "[mdns-dbg] advertise IPv4 = %s\n", inet_ntoa(dbg_a));
    }
