...
static void _rotation_log(const char *msg) {
    char path[MAX_PATH];
                   ...
                   char **response_data, int *response_datalen)
{
    ...
    if (PLIST_IS_DATA(req_eiv_node) && PLIST_IS_DATA(req_ekey_node)) {
        ...
        plist_t res_timing_port_node = plist_new_uint(uxplay_remap_port(timing_lport));
    ...
    if (PLIST_IS_ARRAY(req_streams_node)) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            case 110: {
                ...
                logger_log(raop->logger, LOGGER_ERR, "SETUP 110 mirroring: mirror_data_lport=%u", (unsigned)dport);
                ...
                if (conn->raop_rtp_mirror) {
                    ...
                    if (raop->rotation_pending) {
                        _rotation_log("SETUP 110: clearing rotation_pending, mirror started");
                    }
                    raop->rotation_pending = false;
                ...
                plist_t res_stream_data_port_node = plist_new_uint(uxplay_remap_port(dport));
            ...
            case 96: {
                ...
                plist_t res_stream_node = plist_new_dict();
                plist_t res_stream_data_port_node = plist_new_uint(uxplay_remap_port(dport));
                plist_t res_stream_control_port_node = plist_new_uint(uxplay_remap_port(cport));
                      ...
                      char **response_data, int *response_datalen)
{
    ...
    _rotation_log("TEARDOWN entered: 96=0 110=0 notype=0");
