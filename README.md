...
旋转时 iPhone 的行为：
1. 发送新 SPS/PPS (新分辨率) → `video_report_size()` 回调触发 → 设置 `rotation_pending=true`
2. 断开旧 TCP 连接 → mirror 线程检测 ECONNRESET → 调用 `conn_reset(cls, 1)` 回调
3. **`conn_reset` 无条件设置 `reset_loop=true` + `relaunch_video=true` → 主循环退出 → `video_renderer_destroy()` → 窗口被销毁**

**已实施的修复**:
- `conn_reset()` 中检查 `rotation_pending`，旋转时跳过 pipeline 重建
- TEARDOWN 处理器中 `rotation_pending=true` 时不再调用 `raop_rtp_mirror_stop()` (避免触发 socket close → conn_reset 链)
- 类型 110 TEARDOWN 同样处理

**测试结果**: 5 台设备同时旋转测试，部分设备仍有窗口消失。日志显示 `rotation_pending` 检测正常但仍有设备异常退出。需要进一步排查是否还有其他触发 pipeline 销毁的路径。

**旋转日志位置**: `%LOCALAPPDATA%\rotation.log`
